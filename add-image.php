...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['images']['name'][0])) {
        ...
        foreach ($_FILES['images']['name'] as $key => $filename) {
            ...
            $target_file = $uploadDir . $unique_name;

            if (move_uploaded_file($tmp_name, $target_file)) {
                $conn->query("INSERT INTO gallery_images (image_path) VALUES ('$target_file')");
                $upload_success = true;
            }
        }
    }
...
<style>
  ...
  <body>
    ...
    <nav class="navbar navbar-expand-lg navbar-light sticky-top" data-navbar-on-scroll="data-navbar-on-scroll">
      ...
      <!-- <section> begin -->
      <section class="d-flex align-items-center justify-content-center" style="background:#f6eee2;">
<div class="container">

    <div class="row mt-4 mb-3">
        <div class="col-12 text-center">
            <h2>Add Multiple Gallery Images</h2>
        </div>
    </div>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group mt-4">
            <label><strong>Select Images:</strong></label>
            <input type="file" name="images[]" class="form-control" multiple required>
            <small class="text-muted">You can select multiple images.</small>
        </div>

        <div class="text-center mt-4">
            <button type="submit" class="btn btn-primary btn-submit">Upload Images</button>
        </div>

        <?php if ($upload_success): ?>
            <div class="success-box">Images uploaded successfully!</div>
        <?php endif; ?>
